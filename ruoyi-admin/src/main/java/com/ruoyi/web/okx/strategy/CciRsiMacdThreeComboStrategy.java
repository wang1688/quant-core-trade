...
/**
 * 「CCI+RSI+MACD三指标共振策略」
 * 三大指标同时超卖共振做多，同时超买共振做空，过滤虚假信号
 */
public class CciRsiMacdThreeComboStrategy implements TradeStrategy {
    @Override
    public int signal(KLineVO[] k15m, KLineVO[] k1h, KLineVO[] k4h) {
        ...
        if (rsi < 30 && cci < -100 && macdGold) return 1;
        if (rsi > 70 && cci > 100 && macdDead) return -1;
        return 0;
    }
